/* ============================================================
   DADOS MOCKADOS — Fazenda Fernandina

   Estes são dados fictícios usados para desenvolver a interface
   sem precisar do banco de dados Supabase ainda.

   Na Fase 3, quando o banco estiver configurado, vamos substituir
   esses dados pelas queries reais do arquivo lib/supabase.ts.

   "Mock" = simulação, imitação dos dados reais.
   ============================================================ */

import type { Criacao, Animal } from "./types";

/* -------------------------------------------------------
   As 6 criações da Fazenda Fernandina
   Cada uma vira um card na home e uma página /categoria/[slug]
   ------------------------------------------------------- */
export const CRIACOES_MOCK: Criacao[] = [
  {
    id: "1",
    slug: "cavalos", // aparece na URL: /categoria/cavalos
    nome: "Haras Fernandina",
    categoria: "CAVALOS",
    descricao:
      "Referência nacional na criação do Mangalarga Marchador, com excelência reconhecida na seleção de pampa de preto. A marcha nos define, a excelência nos diferencia.",
    tagline: "A Marcha é o que Nos Move.",
    instagram: "@harasfernandina",
    /* Logo oficial do Haras Fernandina como hero da criação */
    foto_hero: "/haras-fernandina.png",
    criado_em: "2026-01-01",
  },
  {
    id: "2",
    slug: "alpacas",
    nome: "Alpacas dos Andes",
    categoria: "ALPACAS",
    descricao:
      "Criação de alpacas e lhamas com genética de alto padrão, diretamente dos Andes peruanos, 100% puras e rigorosamente selecionadas.",
    tagline: "Alta qualidade genética dos Andes Peruanos.",
    instagram: "@alpacasdosandes",
    /* Logo oficial das Alpacas dos Andes como hero da criação */
    foto_hero: "/alpacas-logo.png",
    criado_em: "2026-01-01",
  },
  {
    id: "3",
    slug: "mini-cabras",
    nome: "Mini Cabras dos Alpes",
    categoria: "MINI CABRAS",
    descricao:
      "Criação de mini cabras com padrão definido, baseada em seleção consistente e manejo de alto nível.",
    tagline: "Pequenas, dóceis e cheias de vida.",
    instagram: "@minicabrasdosalpes",
    foto_hero: "/mini-cabras-logo.png",
    criado_em: "2026-01-01",
  },
  {
    id: "4",
    slug: "mini-porcos",
    nome: "Mini-Porcos da Serra",
    categoria: "MINI PORCOS",
    descricao:
      "Criação de mini porcos com seleção criteriosa e controle de qualidade em cada geração.",
    tagline: "Excelência se mantém com consistência.",
    instagram: "@miniporcosdaserra",
    foto_hero: "/mini-porcos-logo.png",
    criado_em: "2026-01-01",
  },
  {
    id: "5",
    slug: "coelhos",
    nome: "Coelhos da Montanha",
    categoria: "COELHOS",
    descricao:
      "Criação especializada de coelhos, com foco em genética, padrão e qualidade de linhagem.",
    tagline: "Consistência se constrói geração após geração.",
    instagram: "@coelhosdamontanha",
    foto_hero: "/coelhos-logo.png",
    criado_em: "2026-01-01",
  },
];

/* -------------------------------------------------------
   Animais de exemplo para desenvolvimento
   ------------------------------------------------------- */
export const ANIMAIS_MOCK: Animal[] = [
  // ---- CAVALOS — ÉGUAS ----
  {
    id: "a1",
    criacao_id: "1",
    nome: "Bela",
    raca: "Quarto de Milha",
    idade: "2 anos",
    sexo: "Fêmea",
    pelagem: "Alazã",
    grupo: "Éguas",
    status: "reservado",
    andamento: null,
    genealogia_pai: "Whiz N Shine",
    genealogia_mae: "Miss Poco Bars",
    descricao: "Égua Quarto de Milha de alta performance.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a2",
    criacao_id: "1",
    nome: "Ísis",
    raca: "Árabe",
    idade: "6 anos",
    sexo: "Fêmea",
    pelagem: "Tordilha",
    grupo: "Éguas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: "Ansata Ibn Halima",
    genealogia_mae: "Nefertiti da Arábia",
    descricao: "Égua Árabe com linhagem importada.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a3",
    criacao_id: "1",
    nome: "Serena",
    raca: "Mangalarga",
    idade: "7 anos",
    sexo: "Fêmea",
    pelagem: "Pampa",
    grupo: "Éguas",
    status: "disponivel",
    andamento: "Batida",
    genealogia_pai: "Ouro Fino da Serra",
    genealogia_mae: "Brisa do Vale III",
    descricao: "Égua Mangalarga de marcha batida impecável.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- CAVALOS — GARANHÕES ----
  {
    id: "a4",
    criacao_id: "1",
    nome: "Apollo",
    raca: "Mangalarga Marchador",
    idade: "3 anos",
    sexo: "Macho",
    pelagem: "Pampa de Preto",
    grupo: "Garanhões",
    status: "reproducao",
    andamento: "Batida",
    /* Genealogia preenchida com exemplo realista */
    genealogia_pai: "Ouro Negro XXVIII",
    genealogia_mae: "Prata da Serra IV",
    descricao: "Garanhão campeão regional de marcha batida, referência genética do Haras Fernandina.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a5",
    criacao_id: "1",
    nome: "Thor",
    raca: "Lusitano",
    idade: "4 anos",
    sexo: "Macho",
    pelagem: "Cinza",
    grupo: "Garanhões",
    status: "disponivel",
    andamento: "Picada",
    genealogia_pai: "Dom Sebastião VII",
    genealogia_mae: "Lusitânia Real III",
    descricao: "Garanhão Lusitano importado de linhagem nobre, com marcha picada de alta qualidade.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- CAVALOS — POTROS ----
  {
    id: "a6",
    criacao_id: "1",
    nome: "Diana",
    raca: "Mangalarga",
    idade: "5 meses",
    sexo: "Fêmea",
    pelagem: "Tordilha Manchada",
    grupo: "Potros",
    status: "disponivel",
    andamento: "Batida",
    genealogia_pai: "Apollo",
    genealogia_mae: "Serena",
    descricao:
      "Potrinha Mangalarga de conformação perfeita, filha de pai campeão nacional. Criada à pata da mãe no campo.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a7",
    criacao_id: "1",
    nome: "Zeus",
    raca: "Quarto de Milha",
    idade: "4 meses",
    sexo: "Macho",
    pelagem: "Alazão",
    grupo: "Potros",
    status: "disponivel",
    andamento: null,
    genealogia_pai: "Smart Chic Olena",
    genealogia_mae: "Bela Poco Whiz",
    descricao: "Potro Quarto de Milha com futuro promissor.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a8",
    criacao_id: "1",
    nome: "Heitor",
    raca: "Lusitano",
    idade: "1 ano",
    sexo: "Macho",
    pelagem: "Castanho",
    grupo: "Potros",
    status: "reservado",
    andamento: null,
    genealogia_pai: "Dom Sebastião VII",
    genealogia_mae: "Fátima de Lisboa II",
    descricao: "Potro Lusitano de linhagem nobre.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- CAVALOS — EMBRIÕES ----
  {
    id: "a9",
    criacao_id: "1",
    nome: "Embrião Apollo × Serena I",
    raca: "Mangalarga Marchador",
    idade: "Disponível para implante",
    sexo: "Indefinido",
    pelagem: "A definir",
    grupo: "Embriões",
    status: "disponivel",
    andamento: "Batida",
    /* Genealogia obrigatória para embriões — define o valor do lote */
    genealogia_pai: "Apollo",
    genealogia_mae: "Serena",
    descricao: "Embrião Mangalarga Marchador de marcha batida, cruzamento dos melhores exemplares do Haras Fernandina.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a10",
    criacao_id: "1",
    nome: "Embrião Apollo × Ísis I",
    raca: "Mangalarga × Árabe",
    idade: "Disponível para implante",
    sexo: "Indefinido",
    pelagem: "A definir",
    grupo: "Embriões",
    status: "disponivel",
    andamento: null,
    genealogia_pai: "Apollo",
    genealogia_mae: "Ísis",
    descricao: "Embrião de cruzamento especial Mangalarga Marchador × Árabe, combinação rara de elegância e marcha.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- CAVALOS — COBERTURAS ----
  {
    id: "a11",
    criacao_id: "1",
    nome: "Cobertura Apollo — Temporada 2026",
    raca: "Mangalarga Marchador",
    idade: "Garanhão 3 anos",
    sexo: "Macho",
    pelagem: "Pampa de Preto",
    grupo: "Coberturas",
    status: "disponivel",
    andamento: "Batida",
    /* Para cobertura, a genealogia do garanhão é o principal dado de valor */
    genealogia_pai: "Ouro Negro XXVIII",
    genealogia_mae: "Prata da Serra IV",
    descricao: "Cobertura por monta natural ou IATF com o garanhão Apollo, campeão regional de marcha batida. Vagas limitadas para a temporada 2026.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "a12",
    criacao_id: "1",
    nome: "Cobertura Thor — Temporada 2026",
    raca: "Lusitano",
    idade: "Garanhão 4 anos",
    sexo: "Macho",
    pelagem: "Cinza",
    grupo: "Coberturas",
    status: "disponivel",
    andamento: "Picada",
    genealogia_pai: "Dom Sebastião VII",
    genealogia_mae: "Lusitânia Real III",
    descricao: "Cobertura com o garanhão Thor, Lusitano importado de linhagem nobre. Ideal para cruzamento industrial e esporte.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI CABRAS — FILHOTES ----
  {
    id: "c1",
    criacao_id: "3",
    nome: "Bolinha",
    raca: "Pigmeu",
    idade: "3 meses",
    sexo: "Fêmea",
    pelagem: "Branca e Preta",
    grupo: "Filhotes",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Pigmeu dócil e bem socializada.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "c2",
    criacao_id: "3",
    nome: "Pintinha",
    raca: "Anã Nigeriana",
    idade: "2 meses",
    sexo: "Fêmea",
    pelagem: "Caramelo",
    grupo: "Filhotes",
    status: "reservado",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Anã Nigeriana com coloração rara.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI CABRAS — MATRIZES FÊMEAS ----
  {
    id: "c3",
    criacao_id: "3",
    nome: "Estrela",
    raca: "Pigmeu",
    idade: "2 anos",
    sexo: "Fêmea",
    pelagem: "Preta",
    grupo: "Matrizes",
    status: "reproducao",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Matriz Pigmeu com excelente histórico reprodutivo.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "c4",
    criacao_id: "3",
    nome: "Flor",
    raca: "Anã Nigeriana",
    idade: "3 anos",
    sexo: "Fêmea",
    pelagem: "Branca",
    grupo: "Fêmeas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Fêmea Anã Nigeriana selecionada por conformação e docilidade.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI CABRAS — MACHOS ----
  {
    id: "c5",
    criacao_id: "3",
    nome: "Trovão",
    raca: "Pigmeu",
    idade: "1 ano",
    sexo: "Macho",
    pelagem: "Cinza",
    grupo: "Machos",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Macho Pigmeu de ótima conformação e temperamento equilibrado.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI PORCOS — FILHOTES ----
  {
    id: "d1",
    criacao_id: "4",
    nome: "Bolota",
    raca: "Mini Pig",
    idade: "2 meses",
    sexo: "Fêmea",
    pelagem: "Rosada",
    grupo: "Filhotes",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Mini Pig dócil e bem socializada.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "d2",
    criacao_id: "4",
    nome: "Farofa",
    raca: "Mini Pig",
    idade: "3 meses",
    sexo: "Macho",
    pelagem: "Preta e Branca",
    grupo: "Filhotes",
    status: "reservado",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Mini Pig com pelagem rara e temperamento tranquilo.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI PORCOS — MATRIZES ----
  {
    id: "d3",
    criacao_id: "4",
    nome: "Rosinha",
    raca: "Mini Pig",
    idade: "2 anos",
    sexo: "Fêmea",
    pelagem: "Rosada",
    grupo: "Matrizes",
    status: "reproducao",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Matriz Mini Pig com excelente histórico reprodutivo.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI PORCOS — FÊMEAS ----
  {
    id: "d4",
    criacao_id: "4",
    nome: "Pipoca",
    raca: "Mini Pig",
    idade: "1 ano",
    sexo: "Fêmea",
    pelagem: "Branca",
    grupo: "Fêmeas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Fêmea Mini Pig de porte pequeno e temperamento equilibrado.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- MINI PORCOS — MACHOS ----
  {
    id: "d5",
    criacao_id: "4",
    nome: "Carvão",
    raca: "Mini Pig",
    idade: "1 ano",
    sexo: "Macho",
    pelagem: "Preta",
    grupo: "Machos",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Macho Mini Pig de pelagem escura e constituição robusta.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- COELHOS — FILHOTES ----
  {
    id: "e1",
    criacao_id: "5",
    nome: "Algodão",
    raca: "Angorá",
    idade: "2 meses",
    sexo: "Fêmea",
    pelagem: "Branca",
    grupo: "Filhotes",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Angorá com pelo denso e temperamento dócil.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "e2",
    criacao_id: "5",
    nome: "Canela",
    raca: "Mini Rex",
    idade: "3 meses",
    sexo: "Macho",
    pelagem: "Caramelo",
    grupo: "Filhotes",
    status: "reservado",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Mini Rex com pelagem aveludada e coloração exuberante.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- COELHOS — MATRIZES ----
  {
    id: "e3",
    criacao_id: "5",
    nome: "Neve",
    raca: "Angorá",
    idade: "2 anos",
    sexo: "Fêmea",
    pelagem: "Branca",
    grupo: "Matrizes",
    status: "reproducao",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Matriz Angorá com histórico reprodutivo consistente.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- COELHOS — FÊMEAS ----
  {
    id: "e4",
    criacao_id: "5",
    nome: "Mochi",
    raca: "Holland Lop",
    idade: "1 ano",
    sexo: "Fêmea",
    pelagem: "Cinza",
    grupo: "Fêmeas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Fêmea Holland Lop com orelhas caídas e personalidade afetiva.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- COELHOS — MACHOS ----
  {
    id: "e5",
    criacao_id: "5",
    nome: "Thor",
    raca: "Mini Rex",
    idade: "1 ano",
    sexo: "Macho",
    pelagem: "Preta",
    grupo: "Machos",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Macho Mini Rex de pelagem aveludada e porte compacto.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- ALPACAS — ADULTOS ----
  {
    id: "b1",
    criacao_id: "2",
    nome: "Andes",
    raca: "Huacaya",
    idade: "4 anos",
    sexo: "Macho",
    pelagem: "Branca",
    grupo: "Adultos",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Alpaca Huacaya adulto com fleece denso e uniforme, excelente conformação.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "b2",
    criacao_id: "2",
    nome: "Pachamama",
    raca: "Suri",
    idade: "5 anos",
    sexo: "Fêmea",
    pelagem: "Marrom",
    grupo: "Adultos",
    status: "reproducao",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Matriz Suri com fibra sedosa de alto valor. Mãe de filhotes premiados.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "b5",
    criacao_id: "2",
    nome: "Quilla",
    raca: "Huacaya",
    idade: "3 anos",
    sexo: "Fêmea",
    pelagem: "Cinza",
    grupo: "Adultos",
    status: "reproducao",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Matriz Huacaya de fibra fina e uniforme, selecionada por conformação e temperamento dócil.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- ALPACAS — FILHOTES ----
  {
    id: "b3",
    criacao_id: "2",
    nome: "Neve",
    raca: "Suri",
    idade: "1 ano",
    sexo: "Fêmea",
    pelagem: "Branca",
    grupo: "Filhotes",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Suri de fibra sedosa.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "b4",
    criacao_id: "2",
    nome: "Bruma",
    raca: "Huacaya",
    idade: "6 meses",
    sexo: "Fêmea",
    pelagem: "Cinza",
    grupo: "Filhotes",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Filhote Huacaya com fleece denso e uniforme.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },

  // ---- ALPACAS — LHAMAS ----
  {
    id: "b6",
    criacao_id: "2",
    nome: "Cuzco",
    raca: "Lhama Ccara",
    idade: "3 anos",
    sexo: "Macho",
    pelagem: "Branca",
    grupo: "Lhamas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Lhama Ccara de porte majestoso, diretamente dos Andes peruanos.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
  {
    id: "b7",
    criacao_id: "2",
    nome: "Inti",
    raca: "Lhama Lanuda",
    idade: "4 anos",
    sexo: "Fêmea",
    pelagem: "Marrom",
    grupo: "Lhamas",
    status: "disponivel",
    andamento: null,
    genealogia_pai: null,
    genealogia_mae: null,
    descricao: "Lhama Lanuda com fibra de altíssima qualidade.",
    ativo: true,
    criado_em: "2026-01-01",
    atualizado_em: "2026-01-01",
    fotos_animais: [],
  },
];

/* -------------------------------------------------------
   Categorias de filtro por criação
   Cada criação tem suas próprias categorias de animal
   ------------------------------------------------------- */
export const CATEGORIAS_POR_CRIACAO: Record<string, string[]> = {
  cavalos: ["Éguas", "Garanhões", "Potros", "Embriões", "Coberturas"],
  alpacas: ["Filhotes", "Adultos", "Lhamas"],
  "mini-cabras": ["Filhotes", "Matrizes", "Fêmeas", "Machos"],
  "mini-porcos": ["Filhotes", "Matrizes", "Fêmeas", "Machos"],
  coelhos:       ["Filhotes", "Matrizes", "Fêmeas", "Machos"],
};

/* -------------------------------------------------------
   Descrição longa de cada criação para a página interna
   ------------------------------------------------------- */
export const DESCRICAO_CRIACAO: Record<string, string> = {
  cavalos:
    "Referência nacional na criação do Mangalarga Marchador, com excelência reconhecida na seleção de pampa de preto. Cada animal é cuidadosamente selecionado por conformação, marcha e temperamento, garantindo padrão genético de alto nível em cada geração.",
  alpacas:
    "Criação de alpacas e lhamas com genética de alto padrão, diretamente dos Andes peruanos, 100% puras e rigorosamente selecionadas. Nossa fibra é referência pela densidade, finura e uniformidade.",
  "mini-cabras":
    "Criação de mini cabras com padrão definido, baseada em seleção consistente e manejo de alto nível. Animais dóceis e saudáveis, criados com atenção individualizada.",
  "mini-porcos":
    "Criação de mini porcos com seleção criteriosa e controle de qualidade em cada geração. Animais domésticos especializados com saúde garantida e temperamento equilibrado.",
  coelhos:
    "Criação especializada de coelhos de raças puras, com foco em genética, padrão e qualidade de linhagem. Variedades selecionadas para beleza, saúde e temperamento.",
};

/* -------------------------------------------------------
   Informações de Instagram por criação
   ------------------------------------------------------- */
export const INSTAGRAM_INFO: Record<
  string,
  { handle: string; subtitulo: string; url: string }
> = {
  cavalos: {
    handle: "@harasfernandina",
    subtitulo: "Bastidores, novidades e animais disponíveis.",
    url: "https://instagram.com/harasfernandina",
  },
  alpacas: {
    handle: "@alpacasdosandes",
    subtitulo: "Acompanhe nossas alpacas e lhamas.",
    url: "https://instagram.com/alpacasdosandes",
  },
  "mini-cabras": {
    handle: "@minicabrasdosalpes",
    subtitulo: "Novidades e filhotes disponíveis.",
    url: "https://instagram.com/minicabrasdosalpes",
  },
  "mini-porcos": {
    handle: "@miniporcosdaserra",
    subtitulo: "Acompanhe nossas criações.",
    url: "https://instagram.com/miniporcosdaserra",
  },
  coelhos: {
    handle: "@coelhosdamontanha",
    subtitulo: "Raças e filhotes disponíveis.",
    url: "https://instagram.com/coelhosdamontanha",
  },
};
